...
struct DBM {
    ...
    // Domain
    std::vector<Domain1DObs*> dom;
    
    /********************
     Constructor
     ********************/
    
    DBM(double spacing, map<string, double> init_conds_ixns, map<string,double> init_diff_eq_obs, std::map<string,double> lrs) {
        ...
        Domain1DObs* dom_hX1 = new Domain1DObs(1, species_map.at(1).at("X1"), init_diff_eq_obs.at("X1"), spacing, init_diff_eq_obs.at("X1") - 0.5*spacing);
        Domain1DObs* dom_hY1 = new Domain1DObs(1, species_map.at(1).at("Y1"), init_diff_eq_obs.at("Y1"), spacing, init_diff_eq_obs.at("Y1") - 0.5*spacing);
        dom = std::vector<Domain1DObs*>({dom_hX1,dom_hY1});
        ...
        std::vector<std::shared_ptr<AdjointObsCommonTerm>> common_terms;
        common_terms.push_back(std::make_shared<AdjointObsCommonTerm>(1,species_map.at(1).at("X1"),0,ixns));
        common_terms.push_back(std::make_shared<AdjointObsCommonTerm>(1,species_map.at(1).at("Y1"),1,ixns));

        for (auto ixn: ixns) {
            auto adjoint = make_shared<AdjointObs>("adjoint "+ixn->get_name(),ixn,common_terms);
            ixn->set_adjoint(adjoint);
            
            // Validate
            adjoint->print_validate_diff_eq();
        };
        ...
        int no_dims = 2;
        int side_length = 30;
        latt = make_shared<LatticeTraj>(no_dims,side_length,species_vec.at(0));
        ...
        /*
        latt->set_ixn_between_layers(0, species_map.at(0).at("X"), 1, species_map.at(1).at("Y1"),w_ixns.at(0).at("wAnti"));
        latt->set_ixn_between_layers(0, species_map.at(0).at("Y"), 1, species_map.at(1).at("X1"),w_ixns.at(0).at("wAnti"));
         */
