...
int main() {
    ...
    OptionsWakeSleep_BM_PCD options_wake_sleep;
    ...
    int no_gibbs_steps = 10;
    int no_mean_field_steps = 10;
    double dt = 0.01;
    ...
    for (auto opt_step=1; opt_step<=no_opt_steps; opt_step++) {
        ...
        // Solve
        opt.solve_one_step_bm_pcd_params(dbm.latt, opt_step, 0, no_timesteps_ixn_params, timepoint_start_wake_sleep, no_timesteps_wake_sleep, timepoint_start_adjoint, no_timesteps_adjoint, dt, no_mean_field_steps, no_gibbs_steps, fnames, options, options_wake_sleep);
