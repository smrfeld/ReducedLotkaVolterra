...
int main() {
    ...
    init_conds_ixns["hX"] = -1.94; // 1.94
    init_conds_ixns["hY"] = -1.94;
    init_conds_ixns["bX1"] = -1.94;
    init_conds_ixns["bY1"] = -1.94;
    init_conds_ixns["wXX1"] = 0.0;
    init_conds_ixns["wYY1"] = 0.0;

    map<string,double> init_diff_eq_obs;
    
    init_diff_eq_obs["X1"] = 100.0;
    init_diff_eq_obs["Y1"] = 100.0;
    ...
    double spacing = 1.0;
    auto dbm = DBM(spacing, init_conds_ixns, init_diff_eq_obs, lrs);
    ...
    int timepoint_max = 200;
    for (auto i_batch=1; i_batch<=10; i_batch++) {
        // Add
    ...
    for (auto opt_step=1; opt_step<=no_opt_steps; opt_step++) {
        ...
        // Solve
        opt.solve_one_step_rbm_cd_obs(opt_step, 0, no_timesteps_ixn_params, timepoint_start_wake_sleep, no_timesteps_wake_sleep, timepoint_start_adjoint, no_timesteps_adjoint, dt, no_cd_steps, fnames, options, options_wake_sleep);
        
        // Write
        if (opt_step == 1 || opt_step % 10 == 0) {
